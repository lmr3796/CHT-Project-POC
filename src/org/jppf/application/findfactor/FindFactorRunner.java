...
public class FindFactorRunner {
  ...
  public static void main(final String...args) {
    if(args.length != 2) {
        System.err.println("Usage: java FindFactorRunner THREADS NUMBER_TO_FIND");
        throw new IllegalArgumentException();
    }
    int threads = Integer.parseInt(args[0]);
    long toFind = Long.parseLong(args[1]);
    try {
      ...
      FindFactorRunner runner = new FindFactorRunner();

      // Create a job
      JPPFJob job = runner.createJob(toFind, threads);

      // execute a blocking job
      final List<JPPFTask> results = runner.executeBlockingJob(job);
      long total = 0;
      for(JPPFTask r: results)
          total += ((Long) r.getResult()).longValue();
   ...
   */
  public JPPFJob createJob(long toFind, int threadsToSplit) throws Exception {
    // create a JPPF job
    JPPFJob job = new JPPFJob();

    // give this job a readable unique id that we can use to monitor and manage it.
    job.setName("Find factor");

    // add a task to the job.
    //job.addTask(new MatrixSubMultiplicationJob());

    for(int i = 0 ; i < threadsToSplit ; i++){
        long begin = i * (toFind/threadsToSplit) + 1;
        long end = (i == threadsToSplit - 1) ? toFind + 1 : begin + (toFind/threadsToSplit) + 1;
        job.addTask(new FindFactorInRangeTask(toFind, begin, end));
    }
